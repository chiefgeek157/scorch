...
from account.models import ScorchUser
    ...
    def handle(self, *args, **options):
        for group_name, group_info in _groups.items():
            ...
            for user_name, user_info in _users.items():
                user = None
                if user_info['create']:
                    user, created = ScorchUser.objects.get_or_create(
                    ...
                    try:
                        user = ScorchUser.objects.get(username=user_name)
                        self.stdout.write(
                            f' - Located user {user_name}')
                    except ScorchUser.DoesNotExist:
                        self.stdout.write(
                            self.style.WARNING(
